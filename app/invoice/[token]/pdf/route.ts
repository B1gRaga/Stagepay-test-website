      ...
      <Page size="A4" style={styles.page}>
        ...
        {items.map((item: any, i: number) => (
          <View key={i} style={styles.tableRow}>
            ...
            <Text style={styles.col3}>{fmt(item.unit_price, sym)}</Text>
            <Text style={styles.col4}>{fmt(item.amount, sym)}</Text>
          </View>
        ))}
        ...
        <View style={styles.totalsBlock}>
          ...
          <View style={styles.totalRow}>
            ...
            <Text style={styles.totalValue}>{fmt(invoice.vat_amount, sym)}</Text>
          </View>
          {invoice.deposit_amount > 0 && (
            <View style={styles.totalRow}>
              <Text style={{ ...styles.totalLabel, color: '#F59E0B' }}>Deposit paid</Text>
              <Text style={{ ...styles.totalValue, color: '#F59E0B' }}>−{fmt(invoice.deposit_amount, sym)}</Text>
            </View>
          )}
          <View style={styles.grandTotal}>
            <Text style={styles.grandLabel}>Balance Due</Text>
            <Text style={styles.grandValue}>{fmt(invoice.total, sym)}</Text>
          </View>
        </View>
        ...
        <View style={styles.footer}>
          <Text style={styles.footerText}>Generated by StagePay · stagepay.co.bw</Text>
