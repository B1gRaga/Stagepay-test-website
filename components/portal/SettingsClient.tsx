...
:root{
  ...
  html[data-theme="light"] .settings-layout{box-shadow:0 1px 6px rgba(0,0,0,.07),0 1px 3px rgba(0,0,0,.04);}
...
export function SettingsClient({ initialProfile = null }: { initialProfile?: Profile | null }) {
  ...
  const [branding, setBranding] = useState({
    invoice_theme:       initialProfile?.invoice_theme       ?? 'dark-modern',
    brand_color_primary: initialProfile?.brand_color_primary ?? '',
    brand_color_header:  initialProfile?.brand_color_header  ?? '',
  })
  ...
  const [firm, setFirm] = useState({
    ...
    firm_name:  initialProfile?.firm_name  ?? '',
    phone:      initialProfile?.phone      ?? '',
    address:    initialProfile?.address    ?? '',
    city:       initialProfile?.city       ?? '',
    country:    initialProfile?.country    ?? 'BW',
    vat_number: initialProfile?.vat_number ?? '',
  })

  // Invoice defaults form state
  const [inv, setInv] = useState({
    default_currency: initialProfile?.default_currency ?? 'P',
    default_vat_rate: String(initialProfile?.default_vat_rate ?? 14),
    tax_label:        initialProfile?.tax_label ?? 'VAT',
  })
  ...
  useEffect(() => {
    // Skip if the server already provided initial profile data.
